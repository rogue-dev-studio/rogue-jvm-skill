...
JVM languages (Java, Kotlin, Scala). Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `jvm` or user asks by name
...
2. **Confirm tooling** - Verify `jvm` runtime/CLI/MCP is available; document install if missing.
